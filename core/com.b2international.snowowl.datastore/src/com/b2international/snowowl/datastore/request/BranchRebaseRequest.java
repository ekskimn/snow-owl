...
public final class BranchRebaseRequest extends AbstractBranchChangeRequest<Merge> {
	...
	BranchRebaseRequest(final String sourcePath, final String targetPath, final String commitMessage, String reviewId, Runnable postCommitRunnable) {
		super(Merge.class, sourcePath, targetPath, commitMessageOrDefault(sourcePath, targetPath, commitMessage), reviewId, postCommitRunnable);
	}
	
	@Override
	protected Merge execute(RepositoryContext context, Branch source, Branch target) {
		return context.service(MergeService.class).enqueue(sourcePath, targetPath, commitMessage, reviewId, postCommitRunnable);
	}
}
